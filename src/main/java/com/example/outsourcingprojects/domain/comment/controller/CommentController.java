...
public class CommentController {
    ...
    public GlobalResponse<PageDataDTO<GetCommentResponse>> getCommentHandler(
            @PathVariable Long taskId,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size,
            @RequestParam(defaultValue = "newest") String sort
    ) {
        PageDataDTO<GetCommentResponse> response = commentService.getComment(taskId, page, size, sort);
    ...
    public GlobalResponse<UpdateCommentResponse> updateCommentHandler(@PathVariable Long taskId,
                                                               @PathVariable Long commentId,
                                                               @RequestBody UpdateCommentRequest request
    ) {
        UpdateCommentResponse response = commentService.updateComment(taskId, commentId, request);
    ...
    public GlobalResponse<Void> deleteCommentHandler(@PathVariable Long taskId,
                                              @PathVariable Long commentId,
                                              HttpServletRequest userToken
    ) {
        Long userId = (Long) userToken.getAttribute("userId");
